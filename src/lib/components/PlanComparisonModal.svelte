...
>
	<div
		class="w-full max-w-sm border-2 border-(--ink) bg-(--paper)"
		role="dialog"
		aria-modal="true"
	...
	>
		...
		<div class="flex items-center justify-between border-b-2 border-(--ink) px-6 py-4">
			<h2 class="text-xl font-black text-(--ink)">Upgrade to Pro</h2>
			<button
				type="button"
		...
		<div class="p-6">
			...
			<div class="flex items-center justify-between gap-4">
				<div>
					<p class="text-4xl font-black text-(--ink)">&#8377;149</p>
					<p class="mt-1 text-sm font-bold text-(--muted)">one-time payment</p>
				</div>
			...
			<ul class="mt-6 space-y-3">
				<li class="flex items-center gap-3">
					...
					<span class="text-sm font-bold text-(--ink)">Unlimited published pages</span>
				</li>
				<li class="flex items-center gap-3">
					<span
						class="grid size-6 shrink-0 place-items-center border-2 border-(--green) bg-(--soft-green) text-(--green)"
